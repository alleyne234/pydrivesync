...
def authenticate():
    while True:
        ...
        if os.path.exists(token_path):
            user_input = input("Authentication failed. Type 'del' to delete 'token.json' and retry or 'quit' to exit.\n")
            if user_input.lower() == 'del':
                os.remove(token_path)
        else:
            user_input = input("Authentication failed. Press Enter to retry or 'quit' to exit.\n")
...
def process_instructions(service):    
    ...
    while True:
        ...
        for file_index, file_path in enumerate(instruction_files):
            print(f"{file_index + 1}. {os.path.basename(file_path)}")
        ...
        try:
            ...
            if choice_index < 0 or choice_index >= len(instruction_files):
